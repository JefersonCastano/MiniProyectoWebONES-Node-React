...
const HorarioComponent = () => {
    ...
    const { state, states,
        ...
        datosHorario } = useCommonHorarioContext();
    ...
    const handleSelectCell = (col, row) => {
        ...
        if (franja) setFranjaActual({ ...franja, state: 'existing', franja_dia: col });
        else setFranjaActual({franja_dia: col, franja_hora_inicio: currentHour(row), state: 'new'});
    }
    ...
    const handleDeleteFranja = () => {
        const newHorario = deleteFranjaActual();
    ...
    const getColor = (id) => {
        ...
        if (!colorMap.has(id)) {
            const availableColors = Array.from({ length: 14 }, (_, i) => i + 1).filter(i => !usedColors.has(i));
            if (availableColors.length === 0) return 'color-default';
                    ...
                    ${rowIndex === 0 && colIndex === 6 ? 'friday' : ''}`}
                                        ...
                                        { rowIndex === 0 && weekDays[colIndex - 1] }
                                        {
                                            ...
                                            <ModalBS closeModal={closeModal} setCloseModal={setCloseModal} >
                                                ...
                                                <HorarioForm deleteFranjaActual={deleteFranjaActual} />
                                            </ModalBS>
                                        }
                                    </div>
                                );
